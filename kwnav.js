 ...
 *   이 9개는 전부 "조교도 쓰는 화면"이다. 그래서 원장 전용 화면 8장
 *     상담 문의함 · 알림 보내기 · 과제 제출함 · 인강 신청·해제 · 공부 랭킹 · 방문 통계 · 운영진 관리
 *     · 진단평가 보고서(2026-07-30 추가 — 홈 카드조차 없어 주소를 직접 쳐야 했던 화면)
...
(function () {
  try {
    ...
    var ITEMS = [
      ...
      ['/admin-stats',     '📊 방문 통계'],
      ['/admin-staff',     '🧑‍🏫 운영진 관리'],
      // admin.html 홈에는 이 카드가 없다(19개 카드 어디에도 없음 — 주소를 직접 쳐야 들어갔다).
      // 그래서 이 메뉴 줄이 유일한 입구다. 문구는 페이지 제목(admin-report.html:6)과 같게 맞춘다.
    ...
    // ── 5) 9개 + 라벨 + 8개 = 18줄. 폰에서 화면 밖으로 넘칠 수 있어 스크롤을 허용한다.
