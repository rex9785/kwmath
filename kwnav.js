...
(function () {
  try {
    ...
    var ITEMS = [
      ...
      // 2026-08-04 추가 — 테스트를 수업에서 자주 쓰는데(관우T) 이 메뉴에는 줄이 없어, 다른 관리자 화면에서
      //    건너가려면 홈을 거쳐야 했다. 홈 카드와 같은 순서(테스트 → 보고서)로 바로 앞에 넣는다.
      //    ※ 이건 원장 메뉴다. 학생 메뉴에는 상시 자리를 만들지 않는다(배너/팝업만 — 확정된 설계).
      ['/admin-surveys',   '📝 테스트·설문'],
