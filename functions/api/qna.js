...
//       body: { mode:'ai'|'teacher', isPrivate?:bool, title?:string, question:string }
//       - mode='ai'      : 하루 제한(기본 3개) 내에서 Gemini 즉시 답변 → status='answered'
...
async function askGemini(env, question, studentMeta) {
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      if (!question) return jsonErr('질문 내용을 입력해주세요.');
      if (question.length < 4) return jsonErr('질문을 조금 더 자세히 적어주세요. (4자 이상)');
      ...
      if (mode === 'ai') {
        ...
        const ai = await askGemini(env, question, student);
        if (ai.error) {
          // AI 실패 시 질문이 사라지지 않게 — 선생님 대기글로 저장(횟수 차감 안 함)
          const res = await env.DB.prepare(
            'INSERT INTO qna (student_id, author_phone, author_name, mode, is_private, title, question, answer, answered_by, status, qdate, created_at, answered_at) ' +
            "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
          ).bind(student.id || null, phone, authorName, 'teacher', isPrivate, title, question, null, null, 'pending', today, now, null).run();
          return jsonOk({
        ...
        const res = await env.DB.prepare(
          'INSERT INTO qna (student_id, author_phone, author_name, mode, is_private, title, question, answer, answered_by, status, qdate, created_at, answered_at) ' +
          "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)"
        ).bind(student.id || null, phone, authorName, 'ai', isPrivate, title, question, ai.text, 'AI', 'answered', today, now, now).run();
