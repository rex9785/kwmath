...
async function askGemini(env, question, studentMeta, image) {
  ...
  const sys =
    ...
    '2) 교육과정 밖 도구는 절대 쓰지 마라. 특히 역삼각함수(arcsin, arccos, arctan, sin^{-1} 등)·로피탈 정리·테일러 전개·복소수 지수 표현은 금지.\n' +
    ...
    '7) 생각을 그대로 늘어놓지 마라. "아니, 다시 보면…" 같은 되돌이·자기수정·헤매는 과정을 절대 보여주지 말고, 머릿속으로 정리한 뒤 깔끔한 최종 풀이만 제시해라.\n' +
    '8) 마지막 줄에는 반드시 $$\\boxed{최종답}$$ 형태로 최종 답을 분명히 적어 끝맺어라. 절대 중간에서 끊지 말고 답까지 완성해라.\n' +
    '9) 핵심에 집중해 간결하게(불필요하게 길게 늘이지 마라). 따뜻하고 격려하는 말투를 유지해라.\n' +
    '\n' +
    '[기타]\n' +
    '10) 수학·학습과 무관하거나 부적절한 질문이면 정중히 거절하고 수학 질문을 부탁해라. 확실하지 않으면 솔직히 말하고 선생님(관우T)·조교에게 다시 질문하도록 권해라.\n' +
    '11) 사진이 첨부되면 사진 속 문제·풀이·그래프를 꼼꼼히 읽고, 먼저 문제를 한 줄로 다시 정리해 학생이 같은 문제인지 확인하게 한 뒤 풀이해라. 사진이 흐리거나 일부만 보이면 보이는 범위에서 최선을 다하되, 안 보이는 부분은 다시 또렷하게 찍어 달라고 정중히 안내해라.';
  ...
  const body = {
    ...
    // → 사고예산(2048) 명시 + 출력상한 8192로 올려(사고 빼고 ~6000 가용) 마지막 답까지 안 잘리게 함.
    generationConfig: { temperature: 0.3, topP: 0.9, maxOutputTokens: 8192, thinkingConfig: { thinkingBudget: 2048 } },
    safetySettings: [
