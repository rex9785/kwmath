...
//   📌 2026-08-05 — 정산 구간이 "전월 1일~말일"에서 **"전월 5일 ~ 지급월 4일(양끝 포함)"** 로 바뀌었다.
//      (같은 날 잠깐 「전월 6일~지급월 5일」이었다가 4일 끊기로 확정. 아래 💰 블록에 이유.)
...
// 💰 2026-08-05 변경 — 지급 대상 구간이 '전월 1~말일'에서 **'전월 5일 ~ 지급월 4일'** 로 바뀌었다.
//   관우T 확정: 5일에 주는 돈은 "**전날(4일)까지** 일한 것"이다. 구간 끝(4일) ≠ 지급일(5일).
//   왜 5일이 아니라 4일에서 끊나:
//    ① 구간 끝이 지급일과 같으면, 5일 아침에 「지급확정」을 누르는 순간 lockedUpto=5일이 되어
//       그날 저녁 근무를 조교가 못 넣는다. 그런데 5일은 다음 구간(6일~)에도 안 들어간다 → 하루가 사라진다.
//    ② 학원 근무는 저녁인데 송금은 낮에 한다. 돈 보내는 시점에 그날 근무시간은 아직 존재하지도 않는다.
//   ✅ 그래서 **5일 알림 금액 = 실제 지급액(최종)** 이 되었다. 5일 근무는 다음 구간으로 자연히 넘어간다.
//   ⚠️ 4일 예고분만 아직 중간 집계다 — 그날(4일) 저녁 근무가 아직 안 들어와 있다.
// ───────────────────────────────────────────────────────────
...
export async function runPayrollReminder(env) {
  ...
  // 지급 대상 = 이번 달 5일에 나가는 급여구간 = 전월 5일 ~ 이번 달 4일.
  //   4일·5일 어느 쪽에 떠도 지급월은 '이번 달'이다(이번 달 5일이 아직 안 지났거나 바로 오늘이므로).
  const payMonth = p.y + '-' + String(p.m).padStart(2, '0');
  const 구간 = periodOf(payMonth);       // { start:'전월 5일', end:'이번달 4일', payday:'이번달 5일', months }
  const tgtMonth = payMonth;             // 로그·상태 키 호환용 이름 유지
  // 푸시 본문용 짧은 라벨 — '7/5~8/4' 처럼. 앞의 0은 떼서 잠금화면에서 읽기 좋게.
  ...
  // 4일 알림은 그날(4일=구간 마지막 날) 저녁 근무가 아직 안 들어와 있다 → "중간 집계"임을 못박는다.
  // 5일 알림은 구간이 이미 4일에 닫혔으므로 **최종 금액**이다(2026-08-05 구간 변경 효과).
  const 꼬리 = (p.d === 4)
    ? '\n\n· 오늘(4일) 근무는 아직 미반영 (중간 집계)\n· 계좌번호 전체는 알림을 눌러 앱에서 확인'
    : '\n\n· 4일로 마감된 최종액 (오늘 근무는 다음 구간)\n· 계좌번호 전체는 알림을 눌러 앱에서 확인';
  ...
  await logAudit(env, null, {
    ...
    detail: {
      실행일: today + (p.d === 4 ? ' (내일이 월급날 — 예고. 구간 마지막 날인 오늘 근무가 아직 미반영)' : ' (오늘이 월급날 — 구간은 어제 4일로 마감됐으므로 최종액)'),
      지급월: tgtMonth,
      ...
      비고: '계좌번호는 푸시 본문·이 로그 둘 다 뒤 4자리만 남긴다(2026-07-31 변경 — 그 전에는 푸시 본문에 전체가 들어갔다). '
        ...
        + '💰 2026-08-05부터 정산 구간이 「전월 5일~지급월 4일(양끝 포함)」이다(예전엔 전월 1~말일). '
        + '구간 끝(4일)과 지급일(5일)이 하루 다른 건 의도다 — 같은 날로 두면 5일에 지급확정을 누르는 순간 '
        + '그날 근무를 조교가 못 넣는데 다음 구간에도 안 들어가서 하루가 사라진다. '
        + '그래서 5일 알림 금액은 최종액이고, 4일 예고분만 그날(4일) 근무가 빠진 중간 집계다 — 둘이 달라도 정상이다.',
    },
  });
