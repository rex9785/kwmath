...
//
// 💰 급여 구간 (2026-08-05 변경 — 관우T 확정. 같은 날 두 번 바뀌었으니 아래를 그대로 믿을 것):
//   월급날은 매월 5일이고, **지급 전날(4일)까지 일한 것**을 그날 지급한다.
//   ⇒ 정산 구간 = 「전월 5일 ~ 지급월 4일」 (양끝 포함).  예) 2026-08-05 지급 = 2026-07-05 ~ 2026-08-04 근무분
//   ❗ 예전에는 달력 월(1일~말일)이었다. "8월 5일에 7월분(7/1~7/31) 지급" → 지금은 아니다.
//   ❗ 그 다음 잠깐 「전월 6일 ~ 지급월 5일」(지급일 당일 포함)이었다 — **폐기됐다.** 이유는 아래 CUTOFF 주석.
...
// 🔚 구간 마지막 날 = **지급 전날**. 2026-08-05 변경 — 예전엔 지급일 당일(5일)이 구간 끝이었다.
//   왜 바꿨나(관우T 판단): 구간 끝과 월급날이 같은 날이면 두 가지가 깨진다.
//    ① 5일 아침에 「지급확정」을 누르면 lockedUpto=5일이 되어, 그날 저녁 근무를 조교가 못 넣는다.
//       그런데 그 날짜는 다음 구간(6일~)에도 안 들어간다 → **하루가 어느 구간에도 속하지 못하고 사라진다.**
//    ② 학원 근무는 저녁인데 송금은 낮에 한다. 지급하는 순간 그날 근무시간이 아직 존재하지 않는다.
//   4일로 끊으면 5일 아침 리마인드 금액 = 실제 지급액이 되고, 5일 근무는 자연스럽게 다음 구간으로 간다.
export const CUTOFF = PAYDAY - 1;              // = 4. 구간은 이 날까지(포함).
const pad2 = (n) => String(n).padStart(2, '0');

// payMonth('2026-08') → { start:'2026-07-05', end:'2026-08-04', months:['2026-07','2026-08'], payday:'2026-08-05' }
//   payMonth = **지급월**(그 달 5일에 돈이 나간다). 1월이면 전월이 작년 12월이 되는 것까지 여기서 처리.
//   ⚠️ end(구간 끝, 4일) ≠ payday(지급일, 5일). 둘을 섞어 쓰지 말 것.
export function periodOf(payMonth) {
  ...
  return {
    payMonth: y + '-' + pad2(m),
    payday: y + '-' + pad2(m) + '-' + pad2(PAYDAY),        // 돈 나가는 날 = 5일
    start: py + '-' + pad2(pm) + '-' + pad2(CUTOFF + 1),   // 전월 5일 (= 지난 구간 끝 다음날)
    end: y + '-' + pad2(m) + '-' + pad2(CUTOFF),           // 지급월 4일 (지급 전날)
    months: [py + '-' + pad2(pm), y + '-' + pad2(m)],      // 읽어야 할 R2 월파일 2개
  };
}

// 오늘(KST) 기준 "지금 정산할 구간"의 지급월. **돈 기준**이다(= 다음/오늘 송금분).
//   5일 이하 → 이번 달 5일에 나갈 돈 = 전월 5일~이번 달 4일 구간 → 이번 달이 지급월.
//   6일 이상 → 이번 구간은 다음 달 5일에 나간다 → 다음 달이 지급월.
//   ⚠️ 경계가 CUTOFF(4)가 아니라 PAYDAY(5)인 게 맞다 — 지급 당일(5일)에는 오늘 나가는 돈을 봐야 하기 때문.
//      "이 근무일이 어느 구간이냐"(날짜 기준)를 물을 땐 CUTOFF로 갈라야 한다. 두 질문이 하루 어긋난다.
...
// 💰 급여 정산의 표준 계산기 — 한 조교의 한 급여구간(전월 5일~지급월 4일) 합계.
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    if (request.method === 'POST') {
      ...
      //      구간이 두 달에 걸치므로(예: 7/5~8/4) 달 단위로는 8월 전체가 같이 잠겨 버렸다.
      if (!isStaff && body.lock !== undefined) {
        ...
        let 새마감;
        if (body.lock) {
          // 확정: 마감일을 구간 끝(지급월 4일 = 지급 전날)로. 이미 더 뒤까지 확정돼 있으면 뒤로 물리지 않는다.
        ...
        await logAudit(env, request, {
          ...
          detail: {
            ...
            비고: '확정 마감일은 조교 레코드(staff/{번호}.json)의 lockedUpto 한 칸이다. '
              + '구간이 두 달에 걸치므로(전월 5일~지급월 4일) 달 단위 플래그로는 다음 달이 통째로 잠기던 문제를 이 방식으로 없앴다.',
          },
        });
        return Response.json({
      ...
      await logAudit(env, request, {
        ...
        detail: {
          ...
          기기: 기기,
          효과: '이 날짜가 속한 급여구간(전월 5일~지급월 4일) 계산에 바로 반영된다. 시급 × 총시간이 급여이므로 '
            + (이전항목 ? '이 수정폭만큼 지급액이 달라진다.' : '이 시간만큼 지급액이 늘어난다.')
            + ' 지급월은 ' + (Number(date.slice(8, 10)) <= CUTOFF ? month : (date.slice(0, 7) + '의 다음 달')) + ' 5일이다.',
          확정구간수정: 확정범위
