...
// 📅 연도 두 자리는 **「학년도」** 다 (관우T 확정 2026-08-10). 달력 연도가 아니다.
//    `26-1`(3~6월) → `26-s`(7~8월) → `26-2`(9~12월) → `26-w`(26.12~27.2) 네 덩어리가 곧 2026학년도.
//    겨울방학이 해를 넘기지만 **`27-w` 가 아니라 `26-w`** — 그 아이들은 27년 2월까지 아직 26학년도 학년이다.
//    ❌ 기각한 표현: "시작 연도 기준". 겨울 특강을 12월에 열면 26-w, 1월에 열면 27-w 가 되어 해마다 흔들린다.
//    (코드는 이 규칙을 강제하지 않는다 — 아래는 앞 두 자리를 그냥 숫자로 읽을 뿐이라 사람이 지키는 명명 규칙이다.)
      ...
      if (count > 0) {
        ...
        return Response.json({
          error: `[${academy} · ${className}]에 학생 ${count}명이 있어서 삭제할 수 없습니다.${내역}`
            // 2026-08-10 — 「퇴원 처리」라는 안내를 지웠다. 이제 그만두는 학생은 「수료」·「졸업」으로 빠지고,
            //   계정과 기록이 그대로 남는다. 여기서 퇴원을 권하면 관우T를 계정 삭제 버튼으로 안내하는 셈이 된다.
            + (수료 > 0 ? ' 수료생이 있는 반은 지우지 마세요 — 「종강」으로 접으면 목록에서 내려가고 기록은 그대로 남습니다.' : ' (먼저 반을 옮기거나 「수료 처리」하세요)'),
        }, { status: 409 });
      }
